...
class FebboxAPI {
    ...
    async _fetchRaw(url, cookie = null) {
        const headers = {
            ...this.headers,
            ...(cookie ? { cookie: `ui=${cookie}` } : {}),
        };
    ...
    async getIMDBId(shareKey, fid, cookie = null) {
        const url = `${this.baseUrl}/console/file_more_info?fid=${fid}`;
        this._setReferer(shareKey);
        ...
        const imdbElement = doc.querySelector(".imdb");
        const imdb_id = imdbElement
            ? imdbElement.getAttribute("data-imdb-id")
            : null;
    ...
    _extractFileQualities(doc) {
        return Array.from(doc.querySelectorAll(".file_quality")).map(
            (fileDiv) => {
                ...
                const speed = fileDiv
                    .querySelector(".speed span")
                    ?.textContent.trim();
                const size = fileDiv.querySelector(".size")?.textContent.trim();

                return { url, quality, name, speed, size, doc };
            }
        );
    }
}
